...
int main(){
	for(int i=0; i<20; i++){
		set<pair<int,int>> oi;
		for(auto k : krd) for(auto l : crd) oi.insert({k.f+l.f,k.s+l.s});
		for(auto l : oi) krd.insert(l);}
	...
	while(!q.empty()){
		...
		int x=get<0>(aux),y=get<1>(aux),d=get<2>(aux);
		if(val(x,y)&&!vis[x][y]&&grid[x][y]!='#'){
			for(auto k : krd)  if(val(x+k.f,y+k.s) && d+dis[x+k.f][y+k.s]+abs(k.f)+abs(k.s)<=dis[x0][y0]-100) tot++;
			vis[x][y]=true;
			q.push({x+1,y,d+1});
			q.push({x-1,y,d+1});
			q.push({x,y-1,d+1});
